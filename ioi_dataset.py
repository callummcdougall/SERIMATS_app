...
from bert_transformer import get_bert_tokenizer
...
def gen_prompt_uniform(
    templates, names, nouns_dict, N, symmetric, masked, mask_token, prefixes=None, abc=False
):
    nb_gen = 0
    ioi_prompts = []
    while nb_gen < N:
        ...
        # This is the bit where we mask one of the tokens
        assert masked in ["IO1", "IO2", "S1", "S2"]
        if masked == "IO1":
            prompt = prompt.replace("[A]", mask_token, 1)
        elif masked == "IO2":
            prompt = prompt.replace("[A]", mask_token).replace(mask_token, "[A]", 1)
        elif masked == "S1":
            prompt = prompt.replace("[B]", mask_token, 1)
        elif masked == "S2":
            prompt = prompt.replace("[B]", mask_token).replace(mask_token, "[B]", 1)
...
def gen_flipped_prompts(
    ...
    flip_dict = {
        "ABBA": [flip.strip() for flip in abba_flip.split("->")],
        "BABA": [flip.strip() for flip in baba_flip.split("->")]
    }
    ...
    for (prompt, template) in zip(prompts, templates_by_prompt):
        ...
        flip_orig, flip_new = flip_dict[template]

        prompt = copy.copy(prompt)

        used_names = [prompt["S"], prompt["IO"]]
        unused_names = list(set(names) - set(used_names))

        names_dict = {"B": prompt["S"], "A": prompt["IO"]}
        names_dict.update({
            letter: np.random.choice(unused_names) 
            for letter in set(flip_new) - set(flip_orig)
        })

        new_names = [names_dict[new] for new in flip_new]
        # Set prompt["IO"] and prompt["S"], to avoid errors (arbitrary; we'll never actually use these)
        prompt["S"] = new_names[0]
        prompt["IO"] = new_names[1]

        prompt_text_split = prompt["text"].split(" ")

        for i, char in enumerate(prompt_text_split):
            if char in used_names + [mask_token]:
                if char == mask_token: new_names[0] = mask_token
                prompt_text_split[i] = new_names.pop(0)
        
        prompt["text"] = " ".join(prompt_text_split)
...
def get_name_idxs(prompts, tokenizer, idx_types=["IO1", "IO2", "S1", "S2"], masked="", mask_token="[MASK]", prepend_bos=False):
    name_idx_dict = dict((idx_type, []) for idx_type in idx_types)
    for prompt in prompts:
        text_split = ["[CLS]"] + prompt["text"].split(" ") + ["[SEP]"]
        toks = tokenizer.tokenize(" ".join(text_split))
        # Get the first and second instances of IO and S tokens
        # Note we need to respons to which one was masked
        # print(prompt["text"])
        name_idx_dict["IO1"].append(
            toks.index(tokenizer.tokenize(" " + prompt["IO"])[0])
            if masked != "IO1"
            else toks.index(mask_token)
        )
        ...
        # Get the second instance of IO token
        name_idx_dict["IO2"].append(
            len(toks) - toks[::-1].index(tokenizer.tokenize(" " + prompt["IO"])[0]) - 1
            if masked != "IO2"
            else toks.index(mask_token)
        )
...
def get_idx_dict(ioi_prompts, tokenizer, prepend_bos=False, toks=None, masked=""):
    (IO1_idxs, IO2_idxs, S1_idxs, S2_idxs,) = get_name_idxs(
        ioi_prompts,
        tokenizer,
        idx_types=["IO1", "IO2", "S1", "S2"],
        masked=masked,
...
class IOIDataset:
    ...
    def gen_flipped_prompts(self, flip):
        ...
        # Get flipped prompts
        flipped_prompts = gen_flipped_prompts(self.ioi_prompts, self.templates_by_prompt, flip, NAMES, seed, self.mask_token)
