...
def get_heads_and_posns_to_keep(
    ...
    seq_pos_to_keep: Dict[str, Union[str, List[str]]],
    ...
    for layer in range(model.cfg.n_layers):
        ...
        for (head_type, head_list) in circuit.items():
            seq_pos_list = seq_pos_to_keep[head_type]
            if isinstance(seq_pos_list, str):
                seq_pos_list = [seq_pos_list]
            for seq_pos in seq_pos_list:
...
def add_mean_ablation_hook(
    ...
    seq_pos_to_keep: Dict[str, Union[str, List[str]]] = SEQ_POS_TO_KEEP,
